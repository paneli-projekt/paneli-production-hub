...
**Ažurirano: 17. 9. 2026.** · Odluke: `DECISIONS.md` (D-01 … D-93) · Ideje: `IDEJE_KASNIJE.md` (I-01 … I-20) · Povijest do 17. 9.: `STANJE_POVIJEST.md`
...
| | |
|---|---|
| Kod | `30_NOVI_PROGRAM\` (Git repo). Zadnji commit `26a4a88` (17. 9.) sadrži sve do D-91 i skraćeni STANJE; dopuna 8 poslana (`ac7c248`); **D-92 (radne ploče na pili) i D-93 (obrub, Optimizacija, boja gumba) čekaju `GIT_POSALJI.cmd`** |
| Shema | **v17** — migracija ide sama pri pokretanju |
| Testovi | **163** (izbrojano 17. 9. navečer), uvijek s `HUB_TEST_DATA` (README; bez toga se testovi na stvarnim nalozima preskaču) |
| Hub na VM-u | `C:\Paneli\Hub` → `http://192.168.5.201:8766/` (8765 = Knjiga, 8080 = Regal traka); upute `deploy\README_DEPLOY_HUB.md` |
| Zadnji dokument | **33** — radne ploče, ploče stola i zidne obloge na pili (D-92) + dopuna obrub / Optimizacija / boja gumba (D-93); prije toga 32 |
...
| Web ekrani: popis naloga, 1 Unos → 2 Optimizacija → 3 Ponuda → 4 Skladište → 5 Proizvodnja, nabava, šifrarnik, postavke; prijava s lozinkom; uvoz više datoteka; promjene između verzija ponude | `web/`, `api/`, `korisnici.py` | 31, 32 |
...
1. Nastaviti s Igorovim napomenama s prolaza na VM-u (sljedeće u dokument 34).
