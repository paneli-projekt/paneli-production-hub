...
(function (H) {
  ...
  function optBlok(d, opt) {
    ...
    function slika(o, x) { return '<a class="slag-img" href="#" data-pregled="' + x.id + '" data-nm="' + o.nalog_materijal_id + '" title="Pregled slaganja na ekranu"><img src="/api/optimizacija/' + x.id + '/sheme.png?h=150" alt="sheme" onerror="this.parentNode.classList.add(\'nema\')"></a>'; }
    function brojke(x) { return '<span class="big">' + x.broj_ploca + '</span> ploča &nbsp;·&nbsp; isk. <b>' + pct(x.iskoristenje) + '</b> &nbsp;·&nbsp; <b>' + n(x.m2_za_naplatu, 2) + ' m²</b> za naplatu' + (x.rezova ? ' &nbsp;·&nbsp; rezova ' + x.rezova : ""); }
    return opt.map(function (o) {
      ...
      var body;
      if (!o.treba) body = '<div class="bd"><span class="note">materijal se ne slaže na ploču — nema slaganja</span></div>';
      else if (p) body = '<div class="bd slag-bd ok">' + slika(o, p) + '<div class="slag-txt"><div class="slag-naslov ok-t">✔ Potvrđeno slaganje <span class="note">' + esc(p.nacin || "") + '</span></div><div>' + brojke(p) + '</div>' +
        ...
        '<div class="row"><button class="btn pri lg" data-potvrdi-opt="' + glavni.id + '">✔ Potvrdi slaganje</button><button class="btn lg" data-pregled="' + glavni.id + '" data-nm="' + o.nalog_materijal_id + '">Pregled shema</button><a class="btn" href="' + pdf(o, glavni) + '" target="_blank">PDF</a><button class="btn" data-alt="' + o.nalog_materijal_id + '">Alternativa…</button></div></div></div>';
      else body = '<div class="bd slag-bd ceka"><div class="slag-txt"><div class="slag-naslov">Još nema prijedloga slaganja</div><div class="note">Hub izračuna slaganje kakvo pila realno reže (trake → poprečni rezovi → uži komadi); ti ga potvrdiš — ista brojka ide u ponudu i na pilu. Uz njega pokaže i svoju rezervu s najmanje m² ako ona štedi materijal.</div>' +
        '<div class="row"><button class="btn pri lg" data-pred="' + o.nalog_materijal_id + '">Izračunaj slaganje</button><button class="btn" data-alt="' + o.nalog_materijal_id + '">Alternativa…</button></div></div></div>';
      var alt = ostali.length ? '<div class="slag-alt"><div class="note" style="padding:6px 14px 2px">Ostale varijante</div>' + ostali.map(function (x) {
        return '<div class="slag-red"><span>' + lab(x) + ' <span class="note">' + esc(x.nacin || "") + '</span>' + (x.napomena ? ' <span class="note warn-t" title="' + esc(x.napomena) + '">⚠</span>' : "") + '</span><span class="num">' + x.broj_ploca + ' pl · ' + pct(x.iskoristenje) + ' · ' + n(x.m2_za_naplatu, 2) + ' m²' +
  ...
  function optBanner(d, opt, jedan) {
    ...
    if (!ceka.length) return (opt || []).some(function (o) { return o.treba; }) ? '<div class="upoz ok slag-banner"><b>✔ Slaganje ploča potvrđeno</b> ' + (jedan ? "za ovaj materijal" : "za sve materijale") + ' — ponuda i pila rade s tom brojkom.</div>' : "";
    return '<div class="upoz crit slag-banner"><div><b>Slaganje ploča čeka potvrdu</b> — ' + esc(ceka.map(function (o) { return o.materijal; }).join(", ")) + '.<br><span class="note">Bez potvrde nema ponude ni izvoza na pilu. Pogledaj sheme ' + (jedan ? "ispod" : "dolje") + ' i potvrdi' + (jedan ? "" : ", ili potvrdi zadana slaganja (realna za pilu) odjednom") + '.</span></div>' +
      '<span class="grow"></span><button class="btn pri lg" id="potvrdiSve">✔ Potvrdi ' + (jedan ? "zadano slaganje" : "sva zadana slaganja (" + ceka.length + ")") + '</button></div>';
  }
  async function pregledSlaganja(d, o, x) {
    /* pregled slaganja na ekranu (Igor, 17. 9.): sve ploče velike, brojevi i mjere, popis elemenata; PDF samo na klik */
    ...
    var st = p.statistika || {};
    dlg({ naslov: "Slaganje — " + (o.materijal || "") + (x.status === "potvrdjeno" ? " (potvrđeno)" : " (prijedlog)"), wide: true,
      tijelo: '<div class="row" style="gap:16px;flex-wrap:wrap"><span><span class="big">' + (st.ploca || x.broj_ploca) + '</span> ploča ' + mm(p.ploca.L) + ' × ' + mm(p.ploca.W) + '</span><span>isk. <b>' + pct(x.iskoristenje) + '</b></span><span><b>' + n(x.m2_za_naplatu, 2) + ' m²</b> za naplatu</span><span>dijelova ' + n(st.m2_dijelova, 2) + ' m²</span><span>rezova ' + (x.rezova || "—") + '</span><span class="note">' + esc(p.nacin || "") + '</span></div>' + legendaSheme() +
      ...
      gumbi: [{ txt: "Krojni nacrt PDF", on: function () { window.open('/api/nalog/' + d.id + '/ispis/krojni.pdf?materijal=' + o.nalog_materijal_id + '&oid=' + x.id, "_blank"); return false; } }]
        .concat(x.status === "potvrdjeno" ? [] : [{ txt: "✔ Potvrdi ovo slaganje", pri: true, on: async function () { var r = await api("/api/optimizacija/" + x.id + "/potvrdi", { body: {} }); toast(r.ponuda_poslana ? "Potvrđeno — ponuda je već poslana, napravi novu verziju" : "Slaganje potvrđeno", !!r.ponuda_poslana); render(); } }]) });
  }
  function veziOpt(d, opt) {
    ...
    qa("[data-potvrdi-opt]").forEach(function (b) { b.onclick = async function () { b.disabled = true; var r = await api("/api/optimizacija/" + b.dataset.potvrdiOpt + "/potvrdi", { body: {} }); if (r.ponuda_poslana) toast("Ponuda je već poslana — napravi novu verziju", true); else toast("Slaganje potvrđeno"); render(); }; });
    ...
    qa("[data-alt]").forEach(function (b) { b.onclick = function () {
      dlg({ naslov: "Alternativno slaganje", tijelo: '<div class="grid2"><div class="field"><span class="lbl">Način</span><select id="nacin"><option value="auto">Realno za pilu (zadano)</option><option value="hub">Hub rezerva (najmanje m², bez ograničenja pile)</option><option value="uzduzno">Uzdužno</option><option value="poprecno">Poprečno</option><option value="trake">Trake</option></select></div><div class="field"><span class="lbl">Dubina</span><select id="dub"><option value="brzo">brzo</option><option value="najbolje" selected>najbolje</option></select></div></div><div class="note">Zadano slaganje poštuje ograničenja pile iz Postavki (razine rezanja, širine u traci, orijentacija). Hub rezerva pokazuje koliko bi se moglo uštedjeti bez tih ograničenja — potvrdi je samo ako je pila stvarno može izrezati.</div>',
    ...
    if (sve) sve.onclick = async function () {
      ...
      toast(poslana ? "Potvrđeno " + k + " — ponuda je već poslana, napravi novu verziju" : "Potvrđeno slaganje: " + k + " materijal(a)", poslana);
      render();
    };
  }

  // ---------------------------------------------------------------- ekran 3: obračun → ponuda iz Huba
  // ---------------------------------------------------------------- ekran 2: slaganje ploča (korak prije ponude — Igor, 17. 9.)
  E.nalog_slaganje = async function (r) {
    ...
    ljuska({ crumb: crumb(d, "slaganje ploča"), koraci: koraci(d), rail: "nalozi", cls: "c1 stack",
  ...
  E.nalog_pila = async function (r) {
    ...
    var mats = d.materijali.map(function (m) {
      ...
      return '<div class="pane slag-card"><div class="hd"><b>' + esc(m.naziv_kratki || m.naziv_ulaz || "") + '</b><span class="note">' + esc(m.ident || "") + ' · ' + m.elemenata + ' el / ' + m.komada + ' kom · ' + n(m.m2, 2) + ' m²' + (m.debljina ? ' · ' + m.debljina + ' mm' : "") + '</span><span class="grow"></span>' +
        ...
        '<div class="bd slag-bd' + (p ? " ok" : "") + '">' + (p ? '<a class="slag-img" href="#" data-pregled="' + p.id + '" data-nm="' + m.id + '" title="Pregled shema"><img src="/api/optimizacija/' + p.id + '/sheme.png?h=96" alt=""></a><div class="slag-txt"><div><b>' + p.broj_ploca + ' ploča</b> · isk. ' + pct(p.iskoristenje) + ' · ' + n(p.m2_za_naplatu, 2) + ' m² za naplatu · rezova ' + (p.rezova || "—") + ' <span class="note">' + esc(p.nacin || "") + '</span></div>' +
          ...
          : (o.treba === false ? '<span class="note">materijal po dužnom metru — nema slaganja</span>' : '<div class="upoz crit">Slaganje nije potvrđeno — <a href="#/nalog/' + d.id + '/slaganje">potvrdi ga u koraku 2 Slaganje</a>; bez toga nema izvoza na stroj.</div>')) + '</div></div>';
    }).join("");
    ljuska({ crumb: crumb(d, "proizvodnja"), koraci: koraci(d), rail: "nalozi", cls: "c2 proizv",
      ...
      sadrzaj: '<div class="col">' + mats + '</div>' +
        ...
        '<div class="note">Mape: nesting <span class="mono">' + esc(mapaN) + '</span> · pila <span class="mono">' + esc(mapaP) + '</span> <a href="#/postavke">(Postavke)</a>. Na stroj ide samo nalog u statusu potvrđeno / skladište / pila-nesting, bez stavki za potvrdu i s potvrđenim slaganjem.</div><div id="izvozRez"></div></div></div>' +
        '<div class="pane"><div class="hd"><b>Spajanje s drugim nalozima</b><span class="note">isti materijal, zajedno na nesting — tu se bira drukčije slaganje</span></div><div class="bd" id="spajanje"><span class="note">učitavam…</span></div></div></div>',
    ...
    qa("[data-izvoz]").forEach(function (b) { b.onclick = async function () {
      ...
      q("#izvozRez").innerHTML = '<div class="upoz ' + (suho ? "" : "ok") + '"><b>' + (suho ? "Pregled" : "Izvezeno") + ' — ' + esc(vrsta) + '</b> → ' + esc(rr.mapa || mapa) + '<br>' + (rr.paketi || []).map(function (p) { return esc(p.materijal) + ': ' + (p.program ? esc(p.program) + ' · ' : "") + p.elemenata + ' el / ' + p.komada + ' kom' + (p.ploca ? ' · ' + p.ploca + ' ploča' : "") + (p.cix ? ' · ' + p.cix.length + ' CIX' : "") + (p.optimizacija_potvrdjena === false ? ' <span class="warn-t">(slaganje nije potvrđeno)</span>' : ""); }).join("<br>") +
  ...
  E.nalog_proizvodnja = E.nalog_pila;                 // „/pila“ ostaje za stare poveznice
  ...
  E.nabava = async function (r) {
    ...
    ljuska({ crumb: "<b>Nabava</b> · potrebe", rail: "nabava", cls: "c1", akcije: akcije + '<button class="tbtn pri" id="btnIzPotreba">Narudžbenice iz potreba</button>',
      sadrzaj: '<div class="pane"><div class="hd"><b>Ploče — potreba preko svih potvrđenih naloga</b><span class="note">manjak = Σ potrebno − fizičko − naručeno</span></div><div class="bd tight">' + (u.nepoznato.length ? '<div class="upoz">Nepoznata potreba (slaganje nije potvrđeno): ' + u.nepoznato.map(function (x) { return esc(x.nalog) + " / " + esc(x.materijal); }).join("; ") + '</div>' : "") + '<table><thead><tr><th>Ident</th><th>Materijal · nalozi</th><th class="r">Potrebno</th><th class="r">Fizičko</th><th class="r">Rezerv.</th><th class="r">Naručeno</th><th class="r">Manjak</th><th class="r">Restlovi</th></tr></thead><tbody>' + (mats || '<tr><td colspan="8" class="note">nema potvrđenih naloga</td></tr>') + '</tbody></table></div></div>' +
  ...
  H.optBlok = optBlok; H.optBanner = optBanner; H.veziOpt = veziOpt; H.legendaSheme = legendaSheme;   // ekran unosa: kartica „Slaganje ploča“
})(Hub);
